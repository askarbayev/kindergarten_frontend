...
class Admin extends Component{
    ...
    docSave = () => {
        ...
        for (let section of sections){
            ...
            for (let subSection of section.subSections){
                querySubSections.push({title:subSection.title, text:subSection.text})
            }
            querySections.push({title:section.title, text: section.text, subSections: querySubSections})
        }
        ...
        this.props.createDoc(graphqlQuery)
    }
    ...
    textChange = (e, blockType, actionType, textType, section_num, subSection_num)=>{
    ...
    clickAddSection = () => {
        let sections = [...this.state.sections, {title:null, typeTitle:true, text:null, typeText:false, subSections:[]}]
        this.setState({
            sections
        })
    }
    ...
    clickAddSubSection = (section_num) => {
        ...
        let subSections = [...section.subSections, {title:null, typeTitle:true, text:null, typeText:false}]
        ...
        return(
            <div className = "inputElement">
                <Input value = {title} placeholder="Title" 
                    onChange = {(e)=>this.textChange(e, type, 'change', 'title', section_num, subSection_num)}
                    onPressEnter = {(e)=>this.textChange(e, type, 'finish', 'title', section_num, subSection_num)} />
            </div>
        )
        ...
        return (
            <div className = "inputElement">
                <TextArea rows = "4" 
                    ...
                    value = {text}
                    onChange = {(e)=>this.textChange(e, type, 'change', 'text', section_num, subSection_num)}
                    onPressEnter = {(e)=>this.textChange(e, type, 'finish', 'text', section_num, subSection_num)} />
            </div>
        )
    }
    ...
    inputSubSection = (section_num, subSection_num) => {
        ...
        const { title, typeTitle, text, typeText} = this.state.sections[section_num].subSections[subSection_num]
        ...
        return (
            <div className = "sub-section-wrap ">
                ...
                {this.inputTitle(title, 'subSection', section_num, subSection_num, typeTitle)}
                ...
                {this.inputText('subSection', section_num, typeText, text, subSection_num)}
                <div className = "s">
    ...
    inputSection = (section_num) => {
        const { title, typeTitle, text, typeText, subSections} = this.state.sections[section_num]
        const toggle_title_wrap = !typeTitle && title? 'toggle-doc-wrap':'' 
        const toggle_text_wrap = !typeText && text? 'toggle-doc-wrap':'' 
        ...
        return (
            <div className = "section-wrap" onHover>
                ...
                {subSections.map((_, index) =>this.inputSubSection(section_num, index))}

                <div className = "s">
                    {this.inputAddContent('section', section_num)}
                </div>
            </div>
        )
    }
    ...
    inputAddContent = (type, section_num, subSection_num) => {
        if (type === 'main'){
            var select = <div>
                <a className = "popoverAdd" onClick = {(e)=>this.clickAddText('main')}>Text</a><br/>
                <a className = "popoverAdd" onClick = {this.clickAddSection}>Section</a><br/>
                <a className = "popoverAdd" onClick = {this.clickAddSummary}>Summary</a>
            </div>
        }
        else if (type === 'section'){
            var select = <div>
                <a className = "popoverAdd" onClick = {(e)=>this.clickAddText('section', section_num)}>Text</a><br/>
                <a className = "popoverAdd" onClick = {(e)=>this.clickAddSubSection(section_num)}>Sub Section</a><br/>
            </div>
        }
        else if (type === 'subSection'){
            var select = <div>
                <a className = "popoverAdd" onClick = {(e)=>this.clickAddText('subSection', section_num, subSection_num)}>Text</a><br/>
            </div>
        }
        return (
            <Popover
    ...
    actionClick = (blockType, actionType, textType, section_num, subSection_num)=>{
    ...
    docActions = (type, textType, section_num, subSection_num) => {
        return (
            <div className = "text-actions">
                <Icon className = "editIcon" type="edit" onClick = {()=>this.actionClick(type, 'edit', textType, section_num, subSection_num)}/>
                <Icon className = "deleteIcon" type="delete" onClick = {()=>this.actionClick(type, 'delete', textType, section_num, subSection_num)} />
            </div>
        ) 
    }
    ...
    render(){
        ...
        const {title, text, sections, typeText, summary, typeSummary, typeTitle} = this.state
        const toggle_title_wrap = !typeTitle? 'toggle-doc-wrap':''
        const toggle_text_wrap = !typeText && text? 'toggle-doc-wrap':''
        const toggle_summary_wrap = !typeSummary && summary? 'toggle-doc-wrap':''
                ...
                <div className = "centerContent">
                    <div className = "doc-header">
                        <div className = "admin-header">
                            <Button type="primary" onClick = {this.docSave}>Save</Button>
                        </div>
                    </div>
                    ...
                    <div className = "doc-layout">
                        <div className = {`doc-wrap ${toggle_title_wrap}`}>
                            {!typeTitle?<h4 className = "doc-title">Title: {title}</h4>:null}
...
export default connect(mapStateToProps, {createDoc})(Admin)